...
//         int midx2 = Integer.parseInt(midx);
			...
			if (value > 0)
				response.sendRedirect(
						request.getContextPath() + "/EventMan_Mypage/EventMan_Mypage_Main.do?midx=" + midx);
			...
			if (!user.equals("")) {
				...
				if (usertype.equals("member")) {
					...
					out.println("<script>document.location.href='" + request.getContextPath()
							+ "/EventMan_Main/EventMan_Main.do'</script>");
				...
				out.println("<script>document.location.href='" + request.getContextPath()
						+ "/EventMan_Member/EventMan_Member_Login.do'</script>");
			...
			if (id != "") {
				System.out.println("아이디는 널이 아닙니다.");
				id2 = id.substring(2);
			}else {
				System.out.println("아이디는 널 입니다.");
				id2="";
			};
